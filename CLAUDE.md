...
- **`overrides`** — `package.json` pins six transitive deps for security fixes:
  ...
  - `brace-expansion@^5.0.9` — fixes DoS via unbounded intermediate arrays (GHSA-rgw5-rvv9-x895, which bypassed the CVE-2026-14257 mitigation; earlier floor 5.0.8 covered GHSA-mh99-v99m-4gvg) pulled in via `minimatch` (itself an override, below). Keep until the ESLint/minimatch dependency tree ships `brace-expansion@5.0.9+`.
  - `js-yaml@^4.3.1` — fixes quadratic CPU consumption in `!!omap` resolution (GHSA-5p4m-2wfm-xmqj / CVE-2026-59870, unfixed through 4.3.0; earlier floors covered GHSA-h67p-54hq-rp68 and GHSA-52cp-r559-cp3m) pulled in via `@eslint/eslintrc`. Held at `^4` deliberately — `@eslint/eslintrc` declares `^4.1.0`, so the 5.x line is a major jump. Keep until ESLint updates its dependency tree.
